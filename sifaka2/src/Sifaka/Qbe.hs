...
data FuncDef = FuncDef [Linkage] (Maybe AbiTy) [Param] [Block]

instance ToText FuncDef where
    txt (FuncDef linkage ret params blocks) = mconcat [txt l <> " " | l <- linkage] <> "function" <+?> (txt <$> ret) <+> "(" <> mconcat [txt p <> ", " | p <- params] <> ")" <+> "{\n" <> mconcat [txt b | b <- blocks] <> "}"
...
data Block = Block LabelName [Phi] [(LocalName, BaseTy, Inst)] Jump
...
instance ToText Block where
    txt (Block label phis instructions jump) =
        txt label
            ...
            <> mconcat [indentedNL (txt name <+> "=" <> txt ty <+> txt i) | (name, ty, i) <- instructions]
            <> indentedNL (txt jump)

data Module = Module [TypeDef] [DataDef] [FuncDef]
